...
class test_datastore_via_node(unittest.TestCase):
    ...
    def _get_type(self, xpath, value):
        libyang_node = next(
            (
                self.root._context.schemactx.find_path(
                    "/integrationtest:validator/integrationtest:types/integrationtest:"
                    + xpath
                )
            )
        )
    ...
    def test_types_from_node(self):
        ...
        self.assertEqual(
            self._get_type("u_int_32", 4294967295), libyang.schema.Type.UINT32
        )
        self.assertEqual(
            self._get_type("u_int_64", 18446744073709551615), libyang.schema.Type.UINT64
        )
        ...
        self.assertEqual(
            self._get_type("int_32", -2147483648), libyang.schema.Type.INT32
        )
        self.assertEqual(
            self._get_type("int_32", 2147483647), libyang.schema.Type.INT32
        )
        self.assertEqual(
            self._get_type("int_64", -9223372036854775808), libyang.schema.Type.INT64
        )
        self.assertEqual(
            self._get_type("int_64", 9223372036854775807), libyang.schema.Type.INT64
        )
        ...
        self.assertEqual(
            self._get_type("dec_64", 234.234234), libyang.schema.Type.DEC64
        )
